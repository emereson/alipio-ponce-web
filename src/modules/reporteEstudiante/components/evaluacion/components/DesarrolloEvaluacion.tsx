...
import { Clock, Send, AlertCircle, ZoomIn } from "lucide-react";
import {
  Button,
  Card,
  CardBody,
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-row gap-4 items-start">
                ...
                {pregunta.imagen_pregunta && (
                  <div
                    className="shrink-0 cursor-pointer group relative rounded-xl overflow-hidden border-2 border-slate-50 shadow-sm"
                    onClick={() =>
                  ...
                  >
                    <Image
                      ...
                      alt="Apoyo visual"
                      className="object-cover w-20 h-20 md:w-28 md:h-28 transition-transform duration-300 group-hover:scale-110"
                    />
              ...
              {/* CONTENIDO ESPECÍFICO DE RESPUESTAS */}
              <div className="mt-2 md:ml-13">
                {/* 1. ALTERNATIVAS CON SOPORTE PARA IMÁGENES */}
                      ...
                      <div className="flex flex-col gap-3">
                          ...
                          respuestasData.map((r: any, idx: number) => {
                            ...
                            return (
                              <div
                                ...
                                className={`flex items-center gap-4 p-3 sm:px-5 rounded-2xl border transition-all cursor-pointer ${
                                  isSelected
                              ...
                              >
                                <div
                                  className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center font-black text-xs shadow-sm transition-colors ${
                                    isSelected
                                ...
                                <div className="flex flex-row items-center justify-between gap-4 flex-1 min-w-0">
                                  {r.texto && (
                                    <span className="text-sm wrap-break-word leading-snug flex-1 select-none">
                                      {r.texto}
                                    </span>
                                  )}
                                  ...
                                  {r.imagen && (
                                    <div
                                      className="shrink-0 cursor-pointer group relative rounded-xl overflow-hidden"
                                      onClick={(e) => {
                                        e.stopPropagation(); // 🟢 EVITA QUE SE SELECCIONE LA ALTERNATIVA AL HACER CLIC EN LA FOTO
                                        setModalImage(
                                          `${SERVERIMG}/${r.imagen}`,
                                        );
                                      }}
                        ...
                        {tieneError && (
                          <p className="text-red-500 text-xs mt-1 flex items-center gap-1 font-semibold">
                            <AlertCircle size={14} />{" "}
                            {(erroresPregunta?.message as string) ||
                              "Requerido"}
                          </p>
                        )}
                      </div>
                          ...
                          <Controller
                            ...
                            render={({ field }) => (
                              <div className="w-full md:w-70 p-2 bg-white flex items-center">
                                ...
                                >
                                  {respuestasData.distractores?.map(
                                    (d: string) => (
